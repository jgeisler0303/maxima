...
(defun procfortcomm ()
  ...
  ; <col 1>* ... <cr> ;
  (prog (c)
	(gt_while (member (setq c (read-char nil nil '$eof$)) '(c c *))
	       (progn (pprin2 c)
...
(defun procratcomm ()
  ...
  (prog (c)
	(pprin2 '|#|)
	(gt_while (not (eq (setq c (read-char nil nil '$eof$)) *cr*))
	       (pprin2 c))
	(pprin2 *cr*)
...
(defun procccomm ()
  ...
  (prog (c)
	...
	(cond ((eq c '*)
	       (progn (pprin2 c)
		      (setq c (read-char nil nil '$eof$))
		      (repeat (progn (gt_while (not (eq c '*))
					    (progn (pprin2 c)
						   (setq c (read-char nil nil '$eof$))))
				     (pprin2 c)
				     (setq c (read-char nil nil '$eof$)))
...
(defun $readvexp (in)
  ...
  (prog (test oldst st iport)
   ...
   c    (cond ((and (equal test #\*)
	...
	(go loop)
   d    
	(if (equal (car st) #\>) (setq st (rest st)))
        ;(format t "~S~%" (coerce (reverse st) 'string))
        ;(cond ((and (null st)
	;	    (not (member *vexptrm '(#\linefeed #\NULL #\>))))
	;       (go loop)))
	(setq oldst st)
	(cond ((null st) (return nil))
	      ((setq test (let (*prompt-on-read-hang*) (with-input-from-string (stre (coerce (reverse (cons #\$ st)) 'string)) (mread stre nil)))) (return (third test))))
	(setq test (tyi iport))
	(go c)))
